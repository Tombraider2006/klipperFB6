...
## График шейперов сразу в телеграм бот 

[telegram_adxl.md](telegram_adxl.md) читаем - делаем,наслаждаемся.
...
Варианты:
- [Для стоковой головы FB6](filament.cfg)
- [Для biuu_h2](filament_biqu.cfg)
...
1. он генерирует график вибраций для указанной оси с помощью пользовательского скрипта Python.
Затем он переместит график и связанный с ним архив CSV в папку результатов ADXL .
1. он управляет папкой, чтобы удалить старые файлы и сохранить только набор (по умолчанию три) самых последних результатов.
2. Результаты можно найти в папке результатов ADXL , которая находится непосредственно в корне папки конфигурации, чтобы **обеспечить доступ непосредственно из вашего браузера с помощью файлового менеджера FLuidd/Maisail. SSH не нужен!**
...
прочитать про все функции скрипта [тут](https://github.com/Frix-x/klippain/blob/main/docs/features.md)

## Восстановление печати после сбоя питания 

[power_loss.cfg](power_loss.cfg) на данный момент не проверен, стырен [тут](https://github.com/CODeRUS/klipper_config/blob/rumba32/power_loss_simple.cfg)
